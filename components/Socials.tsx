...
function Socials({title,slug,siteUrl}: Props) {
  return (
    <div className="my-4 flex space-x-1 print:hidden">
        ...
        <SocialIcon Icon={FaLinkedin} className="text-white bg-sky-700 py-2.5 px-2.5 w-9 h-9" link={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURI(siteUrl+'/'+slug)}&title=${encodeURI(title)}`}/>
        <SocialIcon Icon={FaTelegram} className="text-white bg-sky-500 py-2.5 px-2.5 w-9 h-9" link={`https://telegram.me/share/url?url=${encodeURI(siteUrl+'/'+slug)}&text=${encodeURI(title)}`}/>
        <SocialIcon Icon={FaEnvelope} className="text-white bg-red-800 py-2.5 px-2.5 w-9 h-9" link={`mailto:?subject=${encodeURI(title)}&body=${encodeURI(siteUrl+'/'+slug)}`}/>
